...
def feature_engineering(data):
    data['years_data']=np.where(((data['age_03'].isna()==False)& (data['age_12'].isna()==False)), 2, 1)
    ...
    data['employment_change']=np.where(data['employment_03']!=data['employment_12'], 1, 0).astype(float)
    ...
    data['edu_change']=np.where(data['edu_gru_03']!=data['edu_gru_12'],1, 0).astype(float)
    ...
    data['issste_change']=np.where(data['issste_03']!=data['issste_12'],1, 0).astype(float)
    ...
    data['searnings']=data[['searnings_03', 'searnings_12']].max(axis=1).astype(float)
    data['rearnings_change']=data['rearnings_12'].fillna(0)-data['rearnings_03'].fillna(0).astype(float)
    data['searnings_change']=data['searnings_12'].fillna(0)-data['searnings_03'].fillna(0).astype(float)
    data.drop(columns=['rearnings_03', 'rearnings_12', 'searnings_03', 'searnings_12', 'issste_03',  'issste_12','urban_03', 'urban_12','edu_gru_03', 'edu_gru_12','bmi_03', 'bmi_12','alc_tob_03', 'alc_tob_12','alcohol_03','tobacco_03','alcohol_12','tobacco_12','hincome_03', 'hincome_12', 'glob_hlth_12', 'glob_hlth_03' ], inplace=True)
    return data
...
def get_final_features(train_data,threshold):
    ...
    final_features=list(correlations[((correlations['composite_score']>.069) | (correlations['composite_score']<-.069))].T.columns.values) 
